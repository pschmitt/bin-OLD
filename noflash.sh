...
#
# Developed on debian wheezy / firefox. No plans to support non-unix systems.
#
# Requires: netcat-openbsd quvi (+ grep awk sed etc.).
# Optional: mplayer firefox youtube-dl greasemonkey (xul-ext-greasemonkey)
# apt-get install netcat-openbsd quvi youtube-dl mplayer xul-ext-greasemonkey
#
###############################################################################
# Parameters
...
YTDL_PAT=`youtube-dl --list-extractors | sort | grep -vE "vimeo|generic" | grep -v ':\|(' | awk '{printf"|%s",tolower($1)}' | sed 's/^|//g'`
  ...
  ( cat $NCFIFO ) | nc -l $NOFLASH_SERVER -p $NOFLASH_PORT | (
    ...
    case $URL in
      ...
      mplayer:*)
        ...
        ;;
      http://*)
        if quvi --verbosity=mute --support $URL ; then
